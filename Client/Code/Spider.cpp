...
//TODO: Check the Create Spider
CSpider::CSpider(LPDIRECT3DDEVICE9 pGraphicDev, const _tchar* pLayerTag, wstring wstrObjName, _vec3 vPos)
	: CMonster(pGraphicDev, pLayerTag, wstrObjName)
{
	...
	m_tHitColl.fRadius = m_fScale*0.2f;
}
...
HRESULT CSpider::Ready_GameObject(void)
{
...
void CSpider::Render_GameObject(void)
{
#ifdef _DEBUG
	Render_HitCollider();
	Render_BoxCollider();
...
HRESULT CSpider::LateReady_GameObject(void)
{
...
HRESULT CSpider::Add_Component(void)
{
	CMonster::Add_Component();
	Engine::CComponent*		pComponent = nullptr;

	pComponent = m_pTextureCom[Animation::MON_IDLE]
		= dynamic_cast<Engine::CTexture*>(Engine::Clone(RESOURCE_STAGE, L"Texture_Spider_Idle"));
	NULL_CHECK_RETURN(pComponent, E_FAIL);
	m_mapComponent[Engine::ID_STATIC].emplace(L"Com_Texture_Spider_Idle", pComponent);

	pComponent = m_pTextureCom[Animation::MON_ATTACK]
		= dynamic_cast<Engine::CTexture*>(Engine::Clone(RESOURCE_STAGE, L"Texture_Spider_Attack"));
	NULL_CHECK_RETURN(pComponent, E_FAIL);
	m_mapComponent[Engine::ID_STATIC].emplace(L"Com_Texture_Spider_Shoot", pComponent);

	pComponent = m_pTextureCom[Animation::MON_DEATH]
		= dynamic_cast<Engine::CTexture*>(Engine::Clone(RESOURCE_STAGE, L"Texture_Spider_Death"));
	NULL_CHECK_RETURN(pComponent, E_FAIL);
	m_mapComponent[Engine::ID_STATIC].emplace(L"Com_Texture_Spider_Death", pComponent);

	return S_OK;
}

void CSpider::Change_Animation(_uint uiAnimationID)
{
...
void CSpider::Animator_GameObject(const _float & fTimeDelta)
{
	if (!m_bDeath)
...
void CSpider::Change_State(Animation::MONSTER State)
{
	m_eCurState = State;
...
_int CSpider::Update_Pattern(_float fTimeDelta)
{
	...
	{
		On_SoundFlag(1, L"Spider_detect_01 #601473.wav");
...
void CSpider::Update_HitCollider()
{
	m_tHitColl.vCenter = { m_vPos.x,m_vPos.y*0.35f,m_vPos.z };

}

void CSpider::Update_BoxCollider()
{
...
_int CSpider::Idle(_float fTimeDelta)
{
...
_int CSpider::Back(_float fTimeDelta)
{
...
_int CSpider::Hit(_float fTimeDelta, _float fDamage)
{
...
_int CSpider::Attack(_float fTimeDelta)
{
	Change_State(Animation::MON_ATTACK);
	if ((int)m_tFrame.fFrameCnt == 3 && !m_bAttack)
	{
		m_bAttack = true;
		On_SoundFlag(4, L"Spider_attack_01.wav");
	}
...
_int CSpider::Death(_float fTimeDelta)
{
	Change_State(Animation::MON_DEATH);

	On_SoundFlag(2, L"Spider_death_03.wav");

	return NO_EVENT;

}

CSpider * CSpider::Create(LPDIRECT3DDEVICE9 pGraphicDev, const _tchar* pLayerTag, wstring wstrObjName, _vec3 vPos, _int* pEventCount)
{
	CSpider*	pInstance = new CSpider(pGraphicDev, pLayerTag, wstrObjName, vPos);
